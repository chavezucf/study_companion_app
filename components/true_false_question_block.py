...
def render_true_false_question(profile_manager):
    current_index = st.session_state.true_false_question_index
    questions = st.session_state.true_false_questions_shuffled

    if current_index >= len(questions):
        st.success("You've answered all available questions! Ending session.")
        st.session_state.true_false_session_active = False
    ...
    if not st.session_state.true_false_last_answer_submitted:
        # Show True/False buttons
        col1, col2 = st.columns(2)
        with col1:
            if st.button("True", key="true_false_answer_true"):
                process_answer(True, current_question, profile_manager)
        with col2:
            if st.button("False", key="true_false_answer_false"):
                process_answer(False, current_question, profile_manager)
    else:
        # Show Next Question button
        if st.button("Next Question", key="true_false_next_question"):
            st.session_state.true_false_question_index += 1
            st.session_state.true_false_last_answer_submitted = False
            st.rerun()
...
def process_answer(user_answer, question, profile_manager):
    ...
    st.session_state.true_false_last_answer_submitted = True